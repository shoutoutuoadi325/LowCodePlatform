...
- **设备元模型**：支持动态定义设备类型的属性、操作和事件
- **MQTT通信**：基于标准MQTT协议的设备接入
- **真实设备模拟**：独立运行的设备模拟器，真实通信而非内存对象
- **场景编排**：可视化的场景设计器，支持拖拽式编排
- **微服务架构**：易于扩展和维护
- **可扩展框架**：固定的基本框架，可变的具体设备类型

### 架构亮点

✨ **设备类型建模**：通过元模型动态定义设备类型，无需修改代码  
✨ **真实通信**：设备通过MQTT与平台通信，完全模拟真实场景  
✨ **固定框架+可变设备**：提供统一框架，支持任意设备类型接入  
✨ **提升抽象级别**：不与物理协议强绑定，为真实设备接入做好准备
...
- **Spring Cloud**：微服务支持
- **Spring Integration MQTT**：MQTT消息处理
